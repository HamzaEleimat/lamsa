...
const BookingListScreen: React.FC = () => {
  ...
  const statusColors = {
    [BookingStatus.PENDING]: customColors.warning,
    [BookingStatus.CONFIRMED]: theme.colors.primary,
    [BookingStatus.COMPLETED]: customColors.success,
    [BookingStatus.CANCELLED]: theme.colors.error,
  };

  // Fetch provider ID when component mounts
  useEffect(() => {
    const fetchProviderId = async () => {
      ...
      try {
        ...
        if (providerId) {
          ...
          setErrorMessage(t("provider.profileNotFound"));
        }
      } catch (error) {
        console.error("Error fetching provider ID:", error);
        setErrorMessage(t("provider.errorLoadingProfile"));
      } finally {
  ...
  const loadBookings = useCallback(async () => {
    if (!providerId || providerIdLoading) return;
    ...
    try {
      ...
      const filters =
        statusFilter !== "all"
          ...
          : undefined;
      const providerBookings = await providerBookingService.getProviderBookings(
        providerId,
        filters
      );
  ...
  useEffect(() => {
    if (!providerIdLoading && providerId) {
      loadBookings();
    }
  }, [loadBookings, providerIdLoading, providerId]);
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      Alert.alert(t("common.error"), t("bookings.errorUpdatingStatus"));
    }
  };
        ...
        <Card.Content>
            ...
            >
              {item.status === BookingStatus.PENDING && (
                <>
                  <Menu.Item
                    onPress={() =>
                      handleStatusUpdate(item.id, BookingStatus.CONFIRMED)
                    }
                  ...
                  <Menu.Item
                    onPress={() =>
                      handleStatusUpdate(item.id, BookingStatus.CANCELLED)
                    }
              ...
              {item.status === BookingStatus.CONFIRMED && isPastBooking && (
                <Menu.Item
                  onPress={() =>
                    handleStatusUpdate(item.id, BookingStatus.COMPLETED)
                  }
          ...
          <View style={styles.chipContainer}>
            ...
            {item.paymentMethod && (
              <Chip
                mode="outlined"
                icon={item.paymentMethod === "ONLINE" ? "credit-card" : "cash"}
                style={styles.paymentChip}
              >
                {t(`bookings.payment.${item.paymentMethod.toLowerCase()}`)}
              </Chip>
            )}
          </View>
        </Card.Content>
