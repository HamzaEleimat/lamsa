...
// Provider login with MFA token (after initial login)
router.post(
  '/provider/login-mfa',
  authRateLimiter, // Rate limit auth attempts
  validate([
    ...
    body('mfaToken')
      ...
      .matches(/^[0-9]{6}$/).withMessage('MFA token must be exactly 6 digits'),
  ]),
  (req, res, next) => authController.providerLoginMFA(req, res, next)
);
