...
export class ReviewAnalyticsService {
  ...
  async getDetailedReviews(
    ...
    const detailedReviews: DetailedReview[] = reviews.map(review => {
      ...
      return {
        id: review.id,
        customerName: (review as any).users?.name || 'Anonymous',
        customerAvatar: (review as any).users?.avatar_url,
        serviceName: (review as any).services?.name_en,
  ...
  async getResponseTemplates(providerId: string): Promise<ResponseTemplates> {
    ...
    return {
      ...
      customTemplates: customTemplates?.map((t: any) => ({
        title: t.title,
        template: t.template,
