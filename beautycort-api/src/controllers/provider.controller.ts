...
export class ProviderController {
  ...
  async updateProvider(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Check if user is authorized (must be provider updating own profile)
  ...
  async searchProviders(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Full text search on names and services
      if (searchParams.query) {
        query = query.or(`
          business_name_ar.ilike.%${searchParams.query}%,
          business_name_en.ilike.%${searchParams.query}%,
          description_ar.ilike.%${searchParams.query}%,
          description_en.ilike.%${searchParams.query}%
        `);
      }
  ...
  async deleteProvider(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Check authorization
      if (req.user?.type !== 'provider' || req.user?.id !== id) {
        throw new AppError('Unauthorized to delete this provider profile', 403);
      }
