    ...
    const leaderboard = await Promise.all(
      leaderboardData.map(async (entry, index) => {
        ...
        return {
          ...
          providerName: entry.providers.business_name || 'Unknown',
          businessName: entry.providers.business_name || 'Unknown',
          avatar: entry.providers.avatar_url || '',
          points: pointsData?.total_points || 0,
          level: pointsData?.level || 1,
          rank: index + 1,
          badges: badges?.map(b => b.achievement_name) || [],
          streak: goals?.booking_streak || 0,
          city: entry.providers.city || ''
        };
      })
    );
