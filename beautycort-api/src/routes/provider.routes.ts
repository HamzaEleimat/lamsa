...
router.post(
  ...
  validate([
    ...
    body('phone').notEmpty().matches(/^\+962[7-9]\d{8}$/).withMessage('Valid Jordanian phone number is required'),
...
router.put(
  ...
  validate([
    ...
    body('phone').optional().matches(/^\+962[7-9]\d{8}$/).withMessage('Valid Jordanian phone number is required'),
    body('email').optional().isEmail().normalizeEmail().withMessage('Valid email is required'),
