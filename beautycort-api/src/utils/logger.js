/**
 * Structured Logging Configuration for BeautyCort API
  ...
  init() {
    ...
    if (environment === 'production' || environment === 'staging') {
      ...
      transports.push(new DailyRotateFile({
        filename: 'logs/beautycort-api-%DATE%.log',
      ...
      // Error logs
      transports.push(new DailyRotateFile({
        filename: 'logs/beautycort-api-error-%DATE%.log',
        datePattern: 'YYYY-MM-DD',
        zippedArchive: true,
        maxSize: '20m',
        maxFiles: '30d',
      ...
      transports.push(new DailyRotateFile({
        filename: 'logs/beautycort-api-audit-%DATE%.log',
        datePattern: 'YYYY-MM-DD',
        zippedArchive: true,
        maxSize: '20m',
        maxFiles: '90d',
    ...
    if (process.env.DD_API_KEY) {
      try {
        ...
        transports.push(new datadogTransport({
          ...
          hostname: process.env.DD_HOSTNAME || 'beautycort-api',
    ...
    if (process.env.AWS_REGION && process.env.AWS_ACCESS_KEY_ID) {
      try {
        ...
        transports.push(new CloudWatchTransport({
          logGroupName: `/aws/beautycort-api/${environment}`,
...
module.exports = new BeautyCortLogger();